...
def get_f1_score(targets, preds):
    # 这里的 preds 和 targets 都是离散值，通常会在模型输出后进行阈值判断
    preds = preds.argmax(dim=1)  # 获取预测的类别
    f1 = f1_score(targets.cpu(), preds.cpu(), average='weighted')#(3)macro,(4)micro,(train)weighted
    return f1
